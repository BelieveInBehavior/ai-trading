        ...
        switch (type) {
          ...
          case "result": {
            ...
            upsertStepResult({
              key: "result:final",
              title: "Final result",
              detail: `${finalResult.data_factors.length} data factors · ${finalResult.research_signals.length} research signals · ${(finalResult.buy_signals ?? finalResult.best_signals).length} buys · ${(finalResult.watchlist ?? []).length} watchlist`,
              status: "complete",
              timestamp,
            });
