...
from utils.sector_flow_provider import get_concept_board_data
from models.llm_model import GLOBAL_LLM
from loguru import logger
from utils.date_utils import get_latest_completed_trading_date, get_trading_date_range, get_previous_trading_dates
...
class HotMoneyAkshare(DataSourceBase):
    ...
    def get_concept_data(self, trade_date: str = None) -> pd.DataFrame:
        """获取概念板块资金流数据"""
        try:
            df = get_concept_board_data(
                trade_date=trade_date,
                require_flow=False,
                allow_industry_fallback=True,
            )
            ...
            logger.info(f"获取概念板块数据成功，{len(df)} 条记录，来源: {source}")
            return df
    ...
    async def get_llm_summary(self, trade_date: str) -> dict:
        ...
        try:
            ...
            concept_data = self.get_concept_data(trade_date)
    ...
    def _construct_analysis_text(self, trade_date: str, zt_data: pd.DataFrame, dt_data: pd.DataFrame,
        ...
        if not concept_data.empty:
            ...
            for _, row in top_concepts.iterrows():
                up_raw = pd.to_numeric(row.get('上涨家数', 0), errors='coerce')
                down_raw = pd.to_numeric(row.get('下跌家数', 0), errors='coerce')
                up_count = 0 if pd.isna(up_raw) else int(up_raw)
                down_count = 0 if pd.isna(down_raw) else int(down_raw)
                total_count = up_count + down_count
                change = pd.to_numeric(row.get('涨跌幅', 0), errors='coerce')
                change = 0.0 if pd.isna(change) else float(change)
                source = row.get('数据源', 'unknown')
                line = f"- {row['板块名称']}: 涨跌幅{change:+.2f}%"

                flow = pd.to_numeric(row.get('主力净流入', 0), errors='coerce')
                if not pd.isna(flow) and abs(float(flow)) > 0:
                    line += f", 主力净流入{float(flow)/1e8:+.2f}亿"
                if total_count > 0:
                    up_ratio = up_count / total_count * 100
                    line += f", 上涨率{up_ratio:.0f}%({up_count}/{total_count})"
                line += f", 来源:{source}"
