...
REQUIRED_COLUMNS = [
    'ts_code', 'name', 'industry', 'close', 'pct_chg', 'change', 'vol', 'amount',
    'amplitude', 'high', 'low', 'open', 'pre_close', 'volume_ratio', 'turnover_rate',
    'pe', 'pb', 'total_mv', 'circ_mv', 'total_share', 'float_share'
]


def _build_builtin_fallback_universe() -> pd.DataFrame:
    """No hardcoded data. Returns empty DataFrame — caller should handle the error."""
...
def _ensure_schema(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in REQUIRED_COLUMNS:
        if col not in df.columns:
            df[col] = np.nan if col not in ('ts_code', 'name', 'industry') else ''
    return df


def _fallback_select(stock_df: pd.DataFrame, query: str, limit: int) -> pd.DataFrame:
    """Rule-based fallback when LLM codegen fails."""
    df = stock_df.copy()
    text = (query or '').strip()
    ...
    for key, mapped in industry_alias.items():
        if key in text:
            selected_industry = mapped
            break

    if selected_industry and 'industry' in df.columns:
        industry_df = df[df['industry'].astype(str).str.contains(selected_industry, na=False)]
        if not industry_df.empty:
            df = industry_df

    sort_by = 'total_mv' if '市值' in text else 'pct_chg' if ('涨幅' in text or '强势' in text) else 'total_mv'
    if sort_by not in df.columns:
        sort_by = 'total_mv'

    ascending = '最小' in text or '最低' in text
    filtered = df.sort_values(by=sort_by, ascending=ascending, na_position='last').head(min(20, limit))
    return filtered


def _to_json_records(df: pd.DataFrame) -> str:
    safe_df = df.astype(object).where(pd.notna(df), None)
    return json.dumps(safe_df.to_dict(orient='records'), ensure_ascii=False)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if df1 is None or df1.empty:
                raise Exception("stock_zh_a_spot_em returned empty data")
        except Exception as e:
            last_error = e
            print(f"[stock_selector] Attempt {attempt}/{max_retries} failed: {e}")
        ...
        # Industry data not available in batch from spot API; set default
        # The _fallback_select function handles industry filtering via keyword matching
        df1['industry'] = '未知'
        df1['total_share'] = np.nan
        df1['float_share'] = np.nan

        return _ensure_schema(df1)

    # All retries exhausted
    print(f"[stock_selector] All {max_retries} retries failed: {last_error}")
    return _ensure_schema(pd.DataFrame())
...
async def stock_selector(market: str, query: str, trigger_time: str, limit: int = 10) -> str:
    ...
    try:
        ...
        stock_df = _ensure_schema(stock_df)

        if stock_df.empty:
            return "Error: 无法获取A股实时行情数据（akshare stock_zh_a_spot_em 返回空），请稍后重试或检查网络连接。"
        ...
        if not code_match:
            fallback_df = _fallback_select(stock_df, query, limit)
            return _to_json_records(fallback_df)
        ...
        if filter_stock_df.empty:
            fallback_df = _fallback_select(stock_df, query, limit)
            return _to_json_records(fallback_df)
        
        result_context = _to_json_records(filter_stock_df)
        return result_context
        
    except Exception as e:
        try:
            fallback_df = _fallback_select(_ensure_schema(stock_df), query, limit)
            return _to_json_records(fallback_df)
        except Exception:
