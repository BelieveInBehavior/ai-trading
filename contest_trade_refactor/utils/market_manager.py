...
class MarketManager:
    ...
    def __init__(self, config: MarketManagerConfig):
        self.config = config
        self.cache_dir = preferred_existing_dir(market_manager_dir(), legacy_market_manager_dir())
    ...
    def _get_trade_calendar_cache_file(self) -> Path:
        """Return path to the shared A-share trade calendar cache file.

        Generated by AI Assistant - 2026-08-05 17:25:00
        """
        return self.cache_dir / "trade_calendar.json"

    def _is_trade_calendar_stale(self, trade_dates: list) -> bool:
        """Return True when cached calendar does not cover today.
    ...
    def _save_trade_calendar_cache(self, trade_dates: list, generated_by: str) -> None:
        """Persist trade calendar to local cache.

        Generated by AI Assistant - 2026-08-05 17:25:00
        """
        cache_file = self._get_trade_calendar_cache_file()
    ...
    def get_trade_date(self, market_name: str="CN-Stock", verbose: bool = False):
        ...
        if market_name in ["CN-Stock", "CN-ETF", "CSI300", "CSI500", "CSI1000"]:
            try:
                ...
                if cache_file.exists():
                    ...
                    if trade_dates and not self._is_trade_calendar_stale(trade_dates):
                        if verbose:
                            print(f"从缓存文件获取{market_name}交易日历成功: {len(trade_dates)}个交易日")
                        return trade_dates
                    if trade_dates and verbose:
                        print(
                            f"交易日历缓存已过期(末位 {trade_dates[-1]})，"
                            f"将重新获取..."
                        )
            except Exception as e:
                if verbose:
        ...
        trade_date.sort()
        if market_name in ["CN-Stock", "CN-ETF", "CSI300", "CSI500", "CSI1000"] and trade_date:
            self._save_trade_calendar_cache(trade_date, "Tushare trade_cal")
        return trade_date
    ...
    def _get_us_stock_basic_cache(self):
        """获取美股基本信息缓存，优先使用离线缓存"""
        cache_path = self.cache_dir / 'us_stock_basic_cache.json'
