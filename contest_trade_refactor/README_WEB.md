...
基于 FastAPI + SSE 的实时交易信号分析 Web 应用。
...
# 或分别启动
python web_app.py          # 后端 http://localhost:8000
...
```python
FastAPI + SSE
├── SseTradeCompany: 扩展 SimpleTradeCompany
│   ├── 保留所有原有功能
│   └── 通过 asyncio.Queue 推送实时状态
├── /api/start: 创建分析会话
├── /api/stream/{session_id}: SSE 事件流
└── REST API: 健康检查等
```
...
响应：`{ "session_id": "..." }`，随后通过 `GET /api/stream/{session_id}` 订阅 SSE 事件流。
...
检查防火墙设置，确保端口 8000（后端）和 3000（前端）开放：
