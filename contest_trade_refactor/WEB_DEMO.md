...
### 修改主题颜色
编辑 `frontend/src/` 中的样式文件，例如组件内联样式或全局 CSS。
...
在 `frontend/src/hooks/useTrading.ts` 中处理新的 SSE 事件类型：

```typescript
// 在 EventSource onmessage 中新增 case
case "custom_event":
...
3. **浏览器兼容**: 需要支持 EventSource 的现代浏览器
